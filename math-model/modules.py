...
def WeightInitialize(init_type, weight, activation=None):
    ...
    init_type_dict = {"xavier": XavierInit,
                      "kaiming": KaimingInit, "orthogonal": OrthogonalInit}
...
def FCBlockSequential(in_channels, out_channels, init_type="kaiming", activation=nn.ReLU(), use_batchnorm=False):
    seq = nn.Sequential(*FCBlock(in_channels, out_channels,
                                 init_type, activation, use_batchnorm))
    seq.out_channels = out_channels
    return seq


def ConvBlock(in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, init_type="kaiming", activation=nn.ReLU(), use_batchnorm=False):
    block_conv = []
    block_conv.append(nn.Conv1d(in_channels, out_channels,
...
def ConvBlockSequential(in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, init_type="kaiming", activation=nn.ReLU(), use_batchnorm=False):
    seq = nn.Sequential(*ConvBlock(in_channels, out_channels, kernel_size, stride,
                                   padding, groups, init_type, activation, use_batchnorm))
    seq.out_channels = out_channels
    return seq
...
def DeConvBlock(in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, init_type="kaiming", activation=nn.ReLU(), use_batchnorm=False):
    block_deconv = []
    block_deconv.append(nn.ConvTranspose2d(
...
def DeConvBlockSequential(in_channels, out_channels, kernel_size, stride=1, padding=0, output_padding=0, init_type="kaiming", activation=nn.ReLU(), use_batchnorm=False):
    seq = nn.Sequential(*DeConvBlock(in_channels, out_channels, kernel_size, stride,
                                     padding, output_padding, init_type, activation, use_batchnorm))
    seq.out_channels = out_channels
    return seq
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels=None, kernel_size=3, stride=1,
        ...
        if stride == 1:
            ...
            self.shortcut = ConvBlockSequential(in_channels, out_channels, kernel_size=1, stride=stride,
                                                padding=0, init_type=None, activation=None, use_batchnorm=use_batchnorm)

        # (TODO) use projection

        block = []
        if is_bottleneck:
            block.append(ConvBlockSequential(in_channels=in_channels, out_channels=in_channels // 2, kernel_size=1,
                                             stride=1, padding=0, init_type=init_type, activation=activation, use_batchnorm=use_batchnorm))
            block.append(ConvBlockSequential(in_channels // 2, out_channels // 2, kernel_size, stride,
                                             padding=(kernel_size - 1) // 2, init_type=init_type, activation=activation, use_batchnorm=use_batchnorm))
            block.append(ConvBlockSequential(out_channels // 2, out_channels, kernel_size=1, stride=1,
                                             padding=0, init_type=None, activation=None, use_batchnorm=use_batchnorm))
        else:
            if padding is None:
                padding = (kernel_size - 1) // 2
            block.append(ConvBlockSequential(in_channels, in_channels, kernel_size, stride=stride,
                                             padding=padding, init_type=init_type, activation=activation, use_batchnorm=use_batchnorm))
            block.append(ConvBlockSequential(in_channels, out_channels, kernel_size, stride=1,
                                             padding=padding, init_type=None, activation=None, use_batchnorm=use_batchnorm))
